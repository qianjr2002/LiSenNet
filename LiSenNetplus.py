...
class MaskDecoder(nn.Module):
    def __init__(self, num_features, num_channels=64, out_channel=2, beta=1, cbam_channels=(24, 18, 12)):
        ...
        self.cbam_1 = CBAM(channels=cbam_channels[0], reduction_ratio=6)
        self.cbam_2 = CBAM(channels=cbam_channels[1], reduction_ratio=6)
        self.cbam_3 = CBAM(channels=cbam_channels[2], reduction_ratio=6)


    def forward(self, x, encoder_out_list):
        # print(x.shape) # torch.Size([1, 24, 63, 32])
        x = self.up1(torch.cat([self.cbam_1(x) , encoder_out_list.pop()], dim=1))  # 64
        # print(x.shape) # torch.Size([1, 18, 63, 64])
        x = self.up2(torch.cat([self.cbam_2(x), encoder_out_list.pop()], dim=1))  # 128
        # print(x.shape) # torch.Size([1, 12, 63, 128])
        x = self.up3(torch.cat([self.cbam_3(x), encoder_out_list.pop()], dim=1))  # 256
        # print(x.shape) # torch.Size([1, 6, 63, 256])
...
class LiSenNetPlus(nn.Module):
    def __init__(self, num_channels=16, n_blocks=2, n_fft=512, hop_length=256, compress_factor=0.3, cbam_channels=(24, 18, 12)):
        ...
        self.decoder = MaskDecoder(self.n_freqs, num_channels=num_channels, out_channel=2, beta=1, cbam_channels=cbam_channels)
        #  cbam_channels=(24, 18, 12) # flops:  122.65 MMac params:  75.41 k
        #  cbam_channels=(16, 12, 8)  # flops:  57.46 MMac params:  37.25 k
...
if __name__ == "__main__":
    model_1 = LiSenNetPlus(num_channels=24, n_blocks=2, n_fft=512, hop_length=256, compress_factor=0.3, cbam_channels=(24, 18, 12))
    ...
    # flops:  122.65 MMac params:  75.41 k
    ...
    print('flops: ', flops, 'params: ', params)
    # flops:  57.46 MMac params:  37.25 k
